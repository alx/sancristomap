...
gpxParser.prototype.parse = function (gpxstring) {
    ...
    for (let idx in wpts){
        ...
        if(wpt.querySelector('extensions') != null){
            let icon = "";
            pt.extensions = {}
            const extensions = wpt.querySelector('extensions');

            for (let extIdx in [].slice.call(extensions.children)){
                const child = extensions.children[extIdx]
                const name = child.tagName.replace("osmand:", "");
                if(name == "icon") {
                    icon = child.innerHTML;
                }
                pt.extensions[name] = child.innerHTML;
            }
...
gpxParser.prototype.toGeoJSON = function () {
    ...
    for(idx in this.waypoints) {
        ...
        if(pt.extensions) {
            feature.properties.extensions = pt.extensions;
        }

        if(pt.icon) {
            feature.properties.icon = pt.icon;
        }
